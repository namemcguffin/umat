...
class AssignConf:
    ...
    ft_path: Annotated[Path, cappa.Arg(short="-f", help="output anndata h5ad file path")]
    dt_path: Annotated[
        Path, cappa.Arg(short="-d", help="output feather file path, containing updated transcript cell assignments")
    ]
...
def run():
    conf = cappa.parse(cappa.Command(AssignConf, name=basename(__file__)), completion=False)
